...
try:
    ...
    if num_entries <= 0:
        ...
        # 保存不包含 malfunction 列的文件
        generated_data_no_malfunction = generated_data.drop(columns=['malfunction'])
        generated_data_no_malfunction.to_csv(output_file_name, index=False)
        print(f"生成的数据（不包含 malfunction 列）已保存到 {output_file_name}")
        # 保存包含 malfunction 列的文件
        output_file_name_answer = output_file_name.replace('.csv', '_answer.csv')  # 添加后缀
        generated_data.to_csv(output_file_name_answer, index=False)
        print(f"生成的数据（包含 malfunction 列）已保存到 {output_file_name_answer}")
